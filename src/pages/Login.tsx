...
export function Login() {
  ...
  return <AuthShell title="Welcome back" description="Sign in to continue to your private fitness tracker.">
    <div className="grid gap-5">
      {locationState?.authError && <div role="alert" className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-sm text-foreground">{locationState.authError}</div>}
      ...
      <form onSubmit={submit} className="grid gap-4" noValidate>
        ...
        <Button type="submit" size="lg" className="w-full" disabled={isPending}>{isSubmitting ? 'Signing in…' : 'Sign in'}</Button>
      </form>
