...
function Calendar({ className, classNames, showOutsideDays = true, captionLayout = 'label', buttonVariant = 'ghost', locale, formatters, components, ...props }: ComponentProps<typeof DayPicker> & { buttonVariant?: ComponentProps<typeof Button>['variant'] }) {
  const defaultClassNames = getDefaultClassNames()
  return <DayPicker showOutsideDays={showOutsideDays} className={cn('group/calendar bg-background p-3 [--cell-radius:var(--radius-md)] [--cell-size:--spacing(8)] in-data-[slot=card-content]:bg-transparent in-data-[slot=popover-content]:bg-transparent', className)} captionLayout={captionLayout} locale={locale} formatters={{ formatMonthDropdown: date => date.toLocaleString(locale?.code, { month: 'short' }), ...formatters }} classNames={{
    ...
    caption_label: cn('font-medium select-none', captionLayout === 'label' ? 'cn-calendar-caption text-sm' : 'cn-calendar-caption-label flex items-center gap-1 rounded-(--cell-radius) text-sm [&>svg]:size-3.5 [&>svg]:text-muted-foreground', defaultClassNames.caption_label),
    ...
    Chevron: ({ className, orientation, ...chevronProps }) => orientation === 'left' ? <ChevronLeft className={cn('size-4', className)} {...chevronProps} /> : orientation === 'right' ? <ChevronRight className={cn('size-4', className)} {...chevronProps} /> : <ChevronDown className={cn('size-4', className)} {...chevronProps} />,
